...
| `upload_reference_images` | `project_id: str`, `kind: str = "character"`, `paths: list[str]?`, `urls: list[str]?`, `files: list[{filename, content_base64}]?` | Upload reference images (主题角色 when `kind="character"`, else `scene`). Provide `files` ({filename, content_base64} — real file transfer through the MCP call, no shared filesystem needed), local `paths` readable by the MCP server, and/or `urls` (downloaded server-side); all forwarded as multipart. Character images are the candidates for shot first frames; ≥1 is required before script generation. Returns `{uploaded: [{id, kind, filename}]}`. |
