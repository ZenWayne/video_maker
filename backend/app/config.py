...
class Settings(BaseSettings):
    ...
    tf_cot_prompt: str = (
        ...
        "prompt. If the motion implies only a small change, it is fine for "
        "the end pose to stay close to the starting pose — do NOT invent "
        "large movements that are not in the motion prompt.\n"
    ...
    tf_cot_retry_prompt: str = (
        "Your previous analysis was empty or unusable. Re-analyze the motion "
        "prompt and starting frame, and output a clear, concrete description "
        "of the character's final pose that naturally results from the "
        "motion. The end pose may stay close to the starting pose if the "
        "motion is small.\n\n"
        ...
        "Motion prompt:\n{motion_prompt}\n\n"
        "Output ONLY the final pose description (no preamble, no reasoning)."
    )
    ...
    tf_prompt: str = (
        ...
        "- [Starting Frame] (first image): Use for background, scene "
        "layout, lighting, camera angle, wardrobe colors, and the "
        "character's overall body framing. Apply the analyzed end pose "
        "below for the character's pose.\n"
