...
8. **Timestamp segments for intra-shot timing (core)**: After the Rule 6 fidelity preamble and the Rule 2 camera-framing clause, lay out the action as consecutive timestamp segments written `[MM:SS-MM:SS]`, together covering EXACTLY `00:00` to the shot's `Duration` with no gaps and no overlaps (a 6-second shot's last segment ends at `00:06`, an 8-second shot at `00:08`). Prose timing phrases — "for the first two seconds", "at the very end", "midway" — are IGNORED by Veo; only bracketed segments actually place actions in time. Rules for the segments:
   ...
   - The Rule 5 talking-head physiological cues attach to the speaking segment(s).
