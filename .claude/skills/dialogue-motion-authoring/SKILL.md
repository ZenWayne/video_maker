---
name: dialogue-motion-authoring
description: Use when writing or editing a video_maker project's shot dialogue (台词) or motion prompts (动作) — through the video-maker-dialogue MCP or directly — e.g. authoring lines, filling motion, or replacing a storyboard. Covers per-duration word-count limits, language matching, motion-prompt language, lip-sync, and storyboard structure rules.
---

# Dialogue & Motion Authoring — Constraints

Authoring dialogue and motion for a video_maker project. Read context first
(`get_project`, `get_shot`/`list_shots`), then follow EVERY constraint below.
These are the same constraints the script generator works to — authoring rules,
not suggestions. (The video-maker-dialogue MCP serves this same content via
`get_authoring_guidelines`; source: `backend/mcp_server/authoring_skill.md`.)
...
- **`shot_duration` must be 4, 6, or 8** — these are the only supported durations
  (the only ones with a word-count range and a valid generation length).
...
- You don't set frames through the MCP — author `text`/`motion_prompt` only;
  keyframes are managed in the UI.

## 5. Reference materials

- Character/scene reference images and the reference voice (音色校准) are managed in
  the UI. Read them via `get_project`; you cannot change them through the MCP
  (except `upload_reference_images` when that tool is available). At least one
  character reference image is required before script generation runs.
