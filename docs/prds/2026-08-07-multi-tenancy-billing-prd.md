...
1. 引入账号体系，**一个用户即一个租户**，项目及其全部素材严格归属到用户。
2. 对所有产生上游成本的操作做**计量**，并按**固定定价表**折算为点数。
3. 实现**预付点数**账本：余额不足则拒绝执行，从机制上杜绝欠费。
4. 通过 **Creem** 完成点数充值，覆盖下单、回调入账、退款/拒付冲正。
5. 用户可自助查看余额、消费明细与用量看板。
...
- **F3.1** 每一次计费操作在**完成时**写一条 `usage_records`，字段见 §7.4。
- **F3.2** 计量口径按操作类型固定：

  | 操作 | 计量单位 | 数量来源 |
  |------|----------|----------|
  | 视频生成 | 秒 | `Shot.shot_duration`（4/6/8） |
  | 首帧/尾帧/角色校准 出图 | 张 | 生成的候选图数量 |
  | 剧本生成、分镜生成/重生成 | 次 | 调用次数 |
  | 分镜文本编辑（DeepSeek） | 次 | 调用次数 |
  | 音色转换 | 秒 | 源音频时长 |
  | 内容分析 — 转写 | 分钟（向上取整） | 参考样片总时长 |
  | 内容分析 — brief 生成 | 次 | 调用次数 |
  | 导出合并 | — | 本期免费（纯 CPU） |
  | 存储 | GB·月 | 每日快照聚合 |

- **F3.3** 同时记录**上游真实成本**（`upstream_cost_usd`），仅用于内部毛利报表，不对用户展示。LLM 类从 Langfuse trace 取，Veo/VC 按上游单价 × 数量估算。
...
- **F4.7** 并发安全：`credit_accounts` 更新使用行级锁（`SELECT ... FOR UPDATE`）或乐观锁版本号。
  > **技术风险**：当前是 SQLite（`sqlite+aiosqlite`），并发写能力有限且无真正的行级锁。见 §14「未决问题 Q1」。
...
- **F6.1** 所有触发计费操作的接口，在入队**之前**完成：估价 → 校验余额 → 冻结。任一步失败则不入队。
...
- **F10.3** 超出免费配额的部分按 GB·月 计费（定价见 §10.2）。
- **F10.4** 项目/分析删除时回退计数器。
...
| 操作 | 单位 | 点数 | 折合 | 上游成本估算 |
|------|------|------|------|--------------|
| 视频生成 4 秒 | 次 | 60 | ¥6.0 | ≈ ¥2.9 |
| 视频生成 6 秒 | 次 | 90 | ¥9.0 | ≈ ¥4.3 |
| 视频生成 8 秒 | 次 | 120 | ¥12.0 | ≈ ¥5.8 |
| 首帧 / 尾帧 / 角色校准 出图 | 张 | 7 | ¥0.7 | ≈ ¥0.28 |
| 剧本生成（编剧） | 次 | 5 | ¥0.5 | ≈ ¥0.2 |
| 分镜生成 / 重生成（导演） | 次 | 10 | ¥1.0 | ≈ ¥0.4 |
| 分镜文本编辑（DeepSeek） | 次 | 1 | ¥0.1 | ≈ ¥0.01 |
| 音色转换 | 秒 | 1 | ¥0.1 | 自建 GPU 摊销 |
| 内容分析 — 转写 | 分钟（不足 1 分钟按 1 分钟计） | 12 | ¥1.2/分钟 | 待定，见 Q8 |
| 内容分析 — brief 生成 | 次 | 15 | ¥1.5 | ≈ ¥0.6 |
| 导出合并 | 次 | 0 | 免费 | 纯 CPU |
| 存储（超出 20 GB 部分） | GB·月 | 5 | ¥0.5 | ≈ ¥0.15（COS 标准存储） |

**观察**：视频生成占单项目成本的 90%+。一个 10 分镜、每镜 8 秒的项目 ≈ 1200 点 + 少量图/文，约 ¥125。定价与新手赠送额度都应围绕这个数字设计。
...
### 第 2 期：把钱算清楚（只计量，不收费）
- `usage_records` + 全部计费操作埋点（F3）
- Veo / VC 纳入计量（此前完全空白）
- 定价表与版本机制（F5）
- 管理后台毛利报表（F11.3）

**验收**：跑完一个真实项目，账本能还原出每一次上游调用及其成本，与 Vertex 账单误差 < 5%。
...
| # | 问题 | 说明 | 建议 |
|---|------|------|------|
| **Q1** | **SQLite 承载不了计费** | 当前 `sqlite+aiosqlite`，写并发靠全局锁，无真正行级锁，且 §7.3 的不变式在并发下难以保证。计费账本对一致性的要求远高于现有业务。 | **强烈建议第 3 期之前迁移到 PostgreSQL**。这是本 PRD 最大的技术前置依赖，需要单独决策。 |
| Q2 | 上游单价未经官方校准 | §10 的成本数字来自公开资料，非官方账单。 | 实施前用 Vertex AI 官方定价页 + 一个月真实账单反推校准。 |
| Q3 | 自建 GPU 的 VC 成本如何摊销 | CosyVoice 跑在自有硬件上，没有按次成本。 | 建议按「GPU 小时成本 ÷ 月均转换秒数」摊销，季度复算。 |
| Q4 | Creem 是否支持人民币结算 | 定价表以 ¥ 计，需确认 Creem 的币种支持与实际到账汇率。 | 与 Creem 确认；若仅支持美元，定价表改以 $ 为锚。 |
| Q5 | 新手赠送的滥用 | 邮箱验证挡不住批量注册。 | 第 1 期先只做邮箱验证；若观察到滥用，再加手机号或设备指纹。 |
| Q6 | 失败归因的准确性 | F6.5 要求 worker 显式给出归因，但现有 `_mark_shot_failed()` 只记文本消息。 | 需在第 3 期重构失败路径，引入结构化的 `failure_reason` 枚举。 |
| Q7 | 已生成素材的历史存储成本 | 迁移时不追溯历史消耗，但老项目会持续占用配额。 | 给迁移用户一次性提高配额，或设定清理宽限期。 |
| Q8 | 内容分析的转写走哪条链路 | `content_analyst.py` 的转写实现与上游供应商尚未确认，成本口径为空。 | 实施第 2 期前确认转写 provider（自建 Whisper / 云 ASR），据此定 §10.2 的转写单价。 |
| Q9 | 签名 URL 无法吊销 | COS 预签名 URL 一旦签发，在 TTL 内无法作废。用户删除项目后，已签发的 URL 仍可访问。 | F2.4 收紧 TTL 到 600 秒是主要缓解手段；若需强吊销，得改为后端中转流量，代价是放弃当前"不中转"的架构优势。 |
