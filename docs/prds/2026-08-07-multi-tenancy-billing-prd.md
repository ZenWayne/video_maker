...
- **F3.2** 计量口径：
  ...
  | 操作 | 计量单位 | 数量来源 | 代码位置 |
  |------|----------|----------|----------|
  | **视频生成** | 秒 | `effective_duration`（**不是** `Shot.shot_duration`） | `app/agents/video_generator.py:156` |
  | **图像生成** | 张 | 生成的候选图数量 | `app/services/image_generation.py` |

  > ⚠️ **视频计价必须用 `effective_duration`，不能用 `Shot.shot_duration`**。
  > 代码里是 `effective_duration = 8 if reference_image_paths else shot_duration` —— **使用参考图时时长被强制为 8 秒**，无视分镜自己声明的 4/6 秒。
  > 若按 `shot_duration` 计价，一个带参考图的 4 秒分镜会实际生成并向 Google 付费 8 秒，却只向用户收 4 秒的钱，**每次少收一半**。这是最容易踩的一个计价错位。
  > 对应地，F6 的**预检估价也必须走同一套时长推导**，否则会出现"估价 4 秒、冻结 4 秒的点数、实际扣 8 秒"的冻结不足。建议把这段推导抽成一个共用函数，估价与计量共用，杜绝两处漂移。
...
- **F7.1** 在 Creem 后台把每个点数包配成一个 product，**币种设为 CNY**（Creem 已确认支持人民币结算）；我方 `credit_packs` 表存 `creem_product_id` 与对应点数。
...
> **上游单价已用 Google 官方定价页核实**（2026-08-07，见附录 A）。仍需在上线前用**一个月真实 Vertex AI 账单**反推校准一次，并由财务确认目标毛利率。定价表一经发布即冻结，调价走新版本。

### 10.0 上游真实单价（官方，2026-08-07 核实）

**Veo 3.1 Fast**（`veo-3.1-fast-generate-001`，**音频已含在单价内，不额外收费**）
...
> **本项目走 720p**：`GenerateVideosConfig` 未设置 `resolution` 参数（`video_generator.py:158-162`），Veo 取默认值 720p。**若日后显式开启 1080p，成本上涨 20%，定价表必须同步发新版本。**
>
> 对比参考：Veo 3.1 标准版 $0.40/秒（720p/1080p）、Veo 3.1 Lite $0.05/秒（720p）。当前选的 Fast 档在质量与成本间取平衡。

**Gemini 3.1 Flash Image**（`gemini-3.1-flash-image-preview`，即 Nano Banana 2）

按**输出 token** 计价而非按张，$60 / 1M 输出 token，每张图的 token 数随分辨率变化：

| 输出分辨率 | 输出 token | 单价 |
|-----------|-----------|------|
| 512（~0.25MP） | 747 | $0.045 |
| **1K（~1MP）** | **1120** | **$0.067** ← 按此计 |
| 2K（~4MP） | 1680 | $0.101 |
| 4K（~16MP） | 2520 | $0.15 |

> 虽然上游按 token 计价，**我们对用户仍按「张」收费**——用户不该为看不懂的 token 数买单。只要输出分辨率固定，每张 token 数就是常数，两种口径等价。若日后按分辨率分档出图，需要在定价表里拆成多档。
...
**汇率**：定价推导按 **1 USD = 7.2 CNY**。汇率写进定价版本的元数据，随版本冻结；汇率波动超过 ±5% 时发新定价版本重算。

成本推导（用 §10.0 的官方单价）：

```
视频 8 秒：$0.10/秒 × 8 = $0.80 = ¥5.76
           目标毛利率 ~50% → 售价 ¥11.5 → 取整 120 点（¥12），实际毛利率 52%

图像 1 张：$0.067 = ¥0.48
           目标毛利率 ~50% → 售价 ¥0.96 → 取整 10 点（¥1.0），实际毛利率 52%
```
...
| 操作 | 模型 | 单位 | 点数 | 售价 | 上游成本 | 毛利率 |
|------|------|------|------|------|----------|--------|
| 视频生成 4 秒 | `veo-3.1-fast-generate-001` @720p | 次 | 60 | ¥6.0 | ¥2.88 | 52% |
| 视频生成 6 秒 | 同上 | 次 | 90 | ¥9.0 | ¥4.32 | 52% |
| 视频生成 8 秒 | 同上 | 次 | 120 | ¥12.0 | ¥5.76 | 52% |
| 图像生成（首帧 / 尾帧 / 角色校准 / 候选图） | `gemini-3.1-flash-image-preview` @1K | 张 | 10 | ¥1.0 | ¥0.48 | 52% |

> 视频按**实际生成秒数**（`effective_duration`）计价，不是分镜声明的时长——带参考图的分镜一律按 8 秒收，见 F3.2 的告警。

**免费项（v1 一律记 0 点）**

剧本生成、分镜生成/重生成、分镜文本编辑、音色转换、内容分析（转写 + brief）、导出合并、存储。理由见 §2.2。

**观察**：视频生成占单项目成本的 95%+，只对视频与图像收费在经济上已经覆盖绝大部分成本。一个 10 分镜、每镜 8 秒、配 20 张图的项目 = 10×120 + 20×10 = **1400 点 ≈ ¥140**（上游成本 ¥67.2）。定价与新手赠送额度都应围绕这个数字设计。
...
**新手赠送**：邮箱验证后赠 **150 点**（= 1 个 8 秒分镜 120 点 + 3 张图 30 点，上游成本约 ¥7.2）。足以完整跑通「出图 → 生成 → 预览」一轮，但不足以白嫖一条成片。

**币种**：Creem 已确认支持人民币结算，点数包直接以 ¥ 标价，不经美元换算，用户侧无汇率波动。
...
| **D4** | 上游单价按 **Veo 3.1 Fast 720p $0.10/秒**、**Nano Banana 2 1K $0.067/张** 定，毛利率统一 52%。 | 已用 Google 官方定价页核实（2026-08-07，附录 A）。代码未设 `resolution`，Veo 取默认 720p。 |
| **D5** | 点数包**以人民币直接标价**。 | Creem 已确认支持人民币结算，无需经美元换算，用户侧无汇率波动。 |

### 14.2 未决问题

| # | 问题 | 说明 | 建议 |
|---|------|------|------|
| Q1 | 官方牌价 ≠ 实际账单 | D4 用的是官方牌价，未含 Vertex 的用量折扣、失败重试的重复计费、跨区流量等实际因素。 | 上线前用**一个月真实 Vertex 账单**反推校准一次；毛利率若偏离 52% 超过 5 个百分点则发新定价版本。 |
| Q2 | 分辨率一旦调高，成本跳涨 | 现在走 720p（$0.10/秒）是因为代码没设 `resolution`。若产品侧决定出 1080p，成本涨 20%（$0.12/秒）。 | 把 `resolution` 显式写进配置并纳入定价版本的输入，避免"改了画质没改价"。 |
...
**上游定价（官方来源，2026-08-07 核实）**

- [Gemini API 定价页](https://ai.google.dev/gemini-api/docs/pricing) — Veo 3.1 各档每秒单价（Fast 720p $0.10 / 1080p $0.12 / 4K $0.30，含音频）；Gemini 3.1 Flash Image $0.067/张（1K）
- [Gemini Enterprise Agent Platform 定价页](https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing) — Nano Banana 2 输出 token 单价（$60/1M）与各分辨率 token 数（1K=1120），与上一条独立互证得到同一 $0.067/张
- [Veo 3.1 Fast 模型文档](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/models/veo/3-1-fast-generate-preview)

> 注：多个第三方博客给出 $0.15/秒（含音频）的说法，与官方页的 $0.10/秒（720p，音频已含）冲突。**以官方页为准**。
