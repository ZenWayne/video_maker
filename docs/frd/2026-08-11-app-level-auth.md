...
- **每个来源 IP 每小时最多注册 1 次**（已定），超出返回 `429`
- 用户名长度/字符集校验，拒绝明显的批量生成模式
- 限流状态放 redis（已有），不引入新组件

> 取客户端 IP 时**必须用 `CF-Connecting-IP`**（或经校验的 `X-Forwarded-For`
> 最右可信段）。直接读 socket 地址拿到的会是 Cloudflare 或 cn-ingress 的地址，
> 那样全站共用一个计数器，第一个注册的人就把所有人挡在门外。

> ⚠️ **1 次/小时对共享出口 IP 的真实用户偏严。** 国内移动网络普遍使用
> CGNAT、办公室/校园走 NAT，成百上千人可能共用一个公网 IP —— 同一小时内
> 第二个真实用户就会被拒。
>
> 权衡：因为注册**送 0 点**，批量注册已无利可图，此限流实际只防
> "刷数据库行"这类低危滥用，收益有限而误伤概率不低。
> 建议上线后观察 `429` 的实际触发量：若出现真实用户被挡的反馈，
> 优先放宽到"每 IP 每天 N 次"而不是继续收紧。
...
| **P3** | 建 `stella` 账号 + 数据迁移（FR-8.3 七步）+ 打开强制校验 | `AUTH_ENFORCED=true` | AC-1～AC-13 全通过 |

> 开关只控制"未认证是否放行"，**不控制**是否签发会话 —— 否则 P1/P2 无法验证。
>
> ⚠️ 实现后又多了一条例外：**计费操作始终要求身份，不受本开关控制**（§10.2 ①）。
> 因此 P1/P2 的"线上行为与现在完全一致"只对**读**成立。
>
...
一条 8 秒分镜（120 点）≈ **24 次**剧本生成（5 点）。所以定价的精力应全部
花在视频上；文本/图像那几项即使估偏一倍也无关大局。

### 定向投喂基准量：**500 点**（已定）

口径是"够跑 4 个分镜"。按线上真实数据推算（2026-08-12 取自生产库
148 个分镜）：
...
```
4 个分镜 × 8 秒 × 15 点/秒 = 480 点
剧本生成 5 + 分镜表 5        =  10 点
                             ─────────
                               490 点  → 取整 500 点（≈ $5）
```

> 取 8 秒而非平均值，是为了让"4 个分镜"在最贵的情况下也够用 ——
> 否则用户选了 8 秒会中途卡住，体验比给少了更糟。
>
> 另：线上每个项目的分镜数多为 **4–6 个**，所以 500 点基本等于
> **一个完整小项目**，作为体验额度是合适的。
...
> ⚠️ **实现后此条语义已变，见 §10.1。** 开启访客模式（`GUEST_USERNAME`）后，
> 未认证的 **GET** 会返回 200 + 演示数据；被拒绝的是写操作（403）与一切计费
> 操作（401）。置空 `GUEST_USERNAME` 可恢复本条的原始语义。
...
- 同一 IP 连续注册两次：第二次返回 `429`
- 伪造 `CF-Connecting-IP` 为不同值时应分别计数（验证取的是该头而非 socket 地址）
- 注册成功的新用户余额为 **0**，且调用任一计费端点返回 `402`（呼应 FR-0）

### AC-12　定向投喂够跑 4 个分镜（§5.1）

给新用户发放 500 点后，应能完整跑通"剧本 → 分镜表 → 4 个 8 秒分镜"，
过程中不出现 `402`；结束后余额应 ≥ 0 且接近 0。

> 这条是对定价口径的端到端校验 —— 若中途 402，说明单价或基准量算错了。
...
| # | 决策 | 影响 |
|---|---|---|
| 12 | **定向投喂基准量 = 500 点**（够 4 个 8 秒分镜 + 剧本/分镜表） | §5.1；按生产库真实时长分布推算 |
| 13 | **注册限流 = 每 IP 每小时 1 次** | FR-10；须用 `CF-Connecting-IP` 取真实 IP |

### 仍待明确

无 —— 需求已完备，可进入实现（P1）。

> 唯一需要边跑边校准的是 §5.1 的单价：上线后用 GCP 账单核对一个真实项目，
> 偏差大就改 `config.yml`。
...
| **限流误伤共享出口的真实用户** | 国内 CGNAT / 办公室 NAT 下，同小时第二个真实用户就注册不了 | FR-10 已标注权衡；上线后观察 `429` 触发量，必要时放宽到按天计 |
| **取错客户端 IP** | 全站共用一个计数器，第一个注册者挡住所有人 | FR-10 明确要求用 `CF-Connecting-IP`，不能读 socket 地址 |
...
## 10. 实现记录与偏差（2026-08-19 补记）

前面 §1–§9 是 2026-08-11/12 定稿时的需求与决策，**原样保留**。本节记录实现完成
后的实际状态：新增了哪些 FRD 里没有的东西、哪几条与 FRD 不一致、为什么。

读 §6 验收标准之前请先读本节 —— 有两条验收的期望值已经变了。

### 10.0 状态

三个阶段的代码都已合并（P1 = PR #52，P2 = PR #53），并于 **2026-08-19** 完成生产
上线：镜像发布、存量数据迁移、访客模式与强制校验开启。

> ⚠️ **本节不记录"当前生产开关是什么值"**，只记录设计与顺序。
>
> 初版 §10 写了一份部署状态清单（"生产仍跑旧镜像""迁移仅在本地执行过"
> "访客模式关闭""开关未开"），**当天之内四条全部失效**。把易变状态写进文档，
> 过时的文档比没有文档更误导。
>
> 要知道生产此刻的实际配置，去看集群，别看这里：
>
> ```bash
> kubectl -n video-maker get configmap video-maker-config \
>   -o jsonpath='{.data.AUTH_ENFORCED}{"  "}{.data.GUEST_USERNAME}{"  "}{.data.MACHINE_TOKEN_USER}{"\n"}'
> ```
>
> ⚠️ 线上 ConfigMap 与 `deploy/k8s/10-configmap.yaml` **曾长期不同步**（该文件从没
> 被 apply 过，若干键线上根本不存在）。改任何一边都要想到另一边：一次
> `kubectl apply -f deploy/k8s/` 会用文件里的值覆盖线上。

### 10.1 新增：访客模式（FRD 未覆盖）

未登录访问者不再一律 401，而是解析成一个**真实的只读账号**（`GUEST_USERNAME`，
本地为 `guest`）。它是普通账号而非特例分支，因此已有约束自动生效：

- owner 过滤 → 只看得见该账号名下的演示数据
- 0 点余额 → 碰不到任何计费操作
- 另加只读位 → 非 GET/HEAD 一律 **403**（余额挡不住删项目、改文案、裁剪这些
  不花钱的写操作）

配套 `app/scripts/seed_guest_demo.py` 把某账号名下的项目整体复制给访客账号。
**副本共用源项目的 COS 对象**（视频动辄几百 MB，复制存储既慢又贵），所以删源
项目会连带删掉演示素材。

刻意**不做成令牌**：浏览器拿不住秘密，发到前端 JS 里的令牌等于公开。服务端兜底
效果相同且前端零改动 —— 关键在 `/api/auth/me` 对访客返回 401，前端据此判定
"未登录"（显示登录入口、不显示余额），而它探测强制校验的请求被访客身份放行，
于是不会把人踢去登录页。

> ⚠️ **这条推翻了 AC-1 的字面表述**（"未认证一律拒绝"）。开启访客模式后，
> 未认证的 **GET** 会返回 200 + 演示数据；被拒绝的是写操作（403）和一切计费
> 操作。`GUEST_USERNAME` 置空则恢复 AC-1 的原始语义。
>
> 访客账号的余额**必须**保持 0 —— 给它发点数等于给所有匿名访问者开计费权限。
> `seed_guest_demo` 会在余额非 0 时拒绝执行。

### 10.2 与 FRD 不一致的地方

**① 计费操作不受 `AUTH_ENFORCED` 控制（偏离 §4.1 的"P1/P2 行为完全一致"）**

匿名调用任何会计费的端点一律 **401**，开关关着也一样。起因是实测发现匿名可以
调 `/rewrite-prompt` 生成运镜提示词而不扣任何点数 —— 等于把一条免费的 LLM 通道
挂在公网上，正是 §1 要解决的问题。分期上线因此只对**读**放宽。

代价：P2 上线后未登录用户无法再触发生成，必须先登录。这与 §4.1 P1/P2 的完成
判据冲突，取"别让匿名烧钱"。

**② 存量迁移按 `owner_id IS NULL` 回填，而非 `creator_name='anonymous'`（偏离 §4.3 步骤 5）**

实测本地库里存量行的 `creator_name` 有 `test-user`、`pw-test`、`test` 等多种值，
按展示字段过滤会漏行，而漏掉的行在开关打开后就是"用户看不见自己的项目"。
`owner_id` 才是权威归属字段，用它做条件也正好与步骤 6 的校验闭环。

**③ 角色校准（`/character-calibrate*`）也扣点数（§5.1 表未列）**

它走 `settings.cc_model`（Gemini 图像）是真实外部调用，既不在 FR-9.1 的 ❌ 行里
也未在 ✅ 行出现。按"漏扣某个 LLM 入口 = 免费通道"处理，按图像单价扣。

**④ `/regenerate-shots` 按视频单价扣，不是 `CREDIT_COST_SHOTLIST`**

该端点实际做的是重置分镜 + 入队 `run_shot_pipeline`（Veo），不是重跑分镜表。
计价一律按实际会调用的外部服务算。

**⑤ 机器令牌未绑账号 = 未鉴权（FRD 未规定）**

`MACHINE_TOKEN` 有效但 `MACHINE_TOKEN_USER` 为空（或绑的账号不存在/停用）时
按未鉴权处理。令牌只能回答"是不是自己人"，回答不了"能看哪些项目""扣谁的点数"；
放行就只能是无边界的全权限，等于配置漏填换来最大权限 —— 与 FR-3 默认拒绝相悖。

> 已定：`MACHINE_TOKEN_USER=stella`。**绑定会连 `is_admin` 一起继承**，因此
> 持有该令牌的人可以调发放接口给任何账号发点数。这是已确认接受的取舍。
> 若要收窄，可在 `require_admin` 里挡掉机器身份（`Principal.is_machine` 现成）。

### 10.3 FRD 未规定、实现中补上的

- **`credits.ensure_balance()` 只读预检**：真正的扣减必须紧贴入队，但端点在入队
  前往往已改过状态机、重置过分镜、甚至归档过 storyboard。只靠 reserve 的话一次
  402 会把项目留在"已推进但没有任务在跑"的半路状态。防透支仍由 reserve 的原子
  扣减负责。
- **越权一律返回 404 而非 403**：403 等于确认"这个 id 存在"。
- **口令先 sha256 再 bcrypt**：bcrypt 只吃前 72 字节，直接喂长口令会被静默截断。
- **CORS 中间件移到最外层**：否则鉴权中间件构造的 401 不带
  `Access-Control-Allow-Origin`，前端只看到跨域错误，读不到"我未登录"。
- **集成测试客户端默认带真实会话**：计费端点不接受匿名调用后，匿名的测试客户端
  测的就不再是真实调用路径。MCP 测试同理，按生产的样子绑定机器令牌。

### 10.4 生产上线流程（2026-08-19 实际执行过一遍）

顺序不能颠倒。第 5 步动的是 `local-path` + `reclaimPolicy=Delete` 的 PVC，
**没有任何备份机制**——执行前务必把库拷一份到集群外（`kubectl cp` 依赖容器里的
`tar`，这个镜像没有，用 `python -c "base64..."` 传出来）。

1. `make deploy-backend` + worker 镜像（退款逻辑在 worker 里）
2. 下发 `MACHINE_TOKEN` secret（当前生产 secret 里没有这个 key）
3. 生产库建 `stella`：`manage_users create stella --admin`
4. 备份 `/app/data/dev.db` + **当天现取**基线 `select count(*) from projects`
5. `migrate_owner_id --owner stella`（先不带 `--apply` 看一眼）→ `--apply`
6. 校验 `owner_id IS NULL` 为 0、总数不变；以 stella 登录确认 `total` 等于基线
7. 最后才把 ConfigMap 的 `AUTH_ENFORCED` 改成 `true` 并滚动重启

> 顺序陷阱：ConfigMap 里 `MACHINE_TOKEN_USER` 已是 `stella`，但生产库里该账号
> 要到第 3 步才存在。第 1 步之后到第 3 步之前，机器令牌处于 fail closed 状态。
> 目前生产未部署 MCP，所以暂不咬人。
>
> 访客模式在生产默认关闭（`GUEST_USERNAME: ""`）；要开需先准备演示数据。

> **实际执行时补充的两条**：
>
> - 前端由 Vercel 跟 git 自动部署，后端是手工发镜像。P2 合并后前端立刻上线、
>   后端还是旧镜像，而新前端不再发 `X-User-Name`、旧后端仍强制要求它——**所有
>   写端点 400，读正常**，页面看着好好的一动手就失败。前后端共享契约变更时，
>   两边的发布必须绑在一起，否则必然漂移。
> - 开启访客模式后，"无效凭据"会被兜底成访客（200 + 演示数据）而不是 401。
>   必须区分"没带凭据"与"带了但无效"，否则 MCP 令牌轮换后会安静地拿到错数据。
>   见 `auth.credentials_presented()`。

### 10.5 两个实测到的运维坑

- **改 `deploy/config.env` 后 `podman restart` 读不到新值，且不报错。**
  `podman compose up -d <service>`、先 `rm -f` 再 `up -d <service>` 同样无效——
  只有整个项目 `down` + `up` 才会重读 env_file。表现为"我改了开关但一点反应
  都没有"。验证要用 `podman exec ... sh -c 'echo $AUTH_ENFORCED'`，别只看文件。
  （k8s 侧不同：改 ConfigMap 后 `kubectl rollout restart` 会重建 Pod。）
- **`uvicorn --reload` 在 bind mount 上不触发**：inotify 跨 podman 挂载传不过来，
  改完后端代码要 `podman restart video-maker-backend-dev` 才生效。
