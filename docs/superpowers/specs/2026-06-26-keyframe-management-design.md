...
- `tf_status`:**保留**(瞬时进度)。
- `skip_tail_frame`:**彻底移除**——从决策/置位逻辑、model/schema/API/前端类型全部删除,并加 `ALTER TABLE shots DROP COLUMN skip_tail_frame` 迁移(`db.py`)。path-as-truth 后它已无任何读点,纯死重量。
- `tf_confirmed`:**从所有决策逻辑中移除**(`resolve_tail_frame` 只看路径);DB 列暂作休眠(仍有写点与前端回显,删列另议)。
...
- 不删 `tf_confirmed` 的 DB 列(休眠即可,删列迁移另议)。`skip_tail_frame` 已彻底删除(含 DROP COLUMN 迁移)。
